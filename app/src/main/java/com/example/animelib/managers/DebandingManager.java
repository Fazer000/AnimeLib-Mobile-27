...
public class DebandingManager {
    ...
    private static final String AGSL_DEBAND_SHADER =
            ...
            "    float n1 = rand(fragCoord);\n" +
            "    float n2 = rand(fragCoord + float2(1.7, 3.1));\n" +
            "    float triDither = (n1 + n2 - 1.0) * (0.012 * in_debandStrength);\n" +
            "    color.rgb = clamp(color.rgb + float3(triDither), 0.0, 1.0);\n" +
            "    return color;\n" +
            "}\n";
